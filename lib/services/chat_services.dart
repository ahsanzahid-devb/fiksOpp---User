...
class ChatServices extends BaseService {
  ...
  Future<void> setOnlineCount(
    ...
    try {
      await contactDocRef.update({"isOnline": status});
    } on FirebaseException catch (e) {
      // When user opens chat before contact docs exist, update() can throw `not-found`.
      if (e.code == 'not-found') {
        await contactDocRef.set({"isOnline": status}, SetOptions(merge: true));
      } else {
        log('setOnlineCount firebase error: ${e.toString()}');
      }
    } catch (e) {
      log('setOnlineCount error: ${e.toString()}');
    }
  }
  ...
  Stream<UserData> isReceiverOnline(
    ...
    return userRef!
      ...
      final isOnline = isOnlineValue is int
          ? isOnlineValue
          : isOnlineValue is num
              ? isOnlineValue.toInt()
              : 0;
      return UserData(isOnline: isOnline);
    });
  }
